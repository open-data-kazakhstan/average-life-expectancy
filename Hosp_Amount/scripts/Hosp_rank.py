...
file2 = r'C:\Users\USER\Desktop\Life_Project\Hosp_Amount\data\Hosp_fin.csv'
...
def ml_new_regions(region_to_test, region_predict):
    ...
    values = df_vko['Число больничных органзаций'].values.tolist()
    k = 0
    c = 0
    ...
    list_res = []
    list_yrs = []
    filtered_df = df.loc[(df['Регион'] == region_predict) & (df['Год'] == 2021), 'Число больничных органзаций']
    ...
    kval = 0
    for i in list_yrs:
        df['Число больничных органзаций'] = np.where((df['Регион'] == region_predict) & (df['Год'] == i), list_res[kval], df['Число больничных органзаций'])
        kval = kval +1
...
li = []
ranks = []
df1 = df1.sort_values(by=['Год', 'Регион', 'Число больничных органзаций'], ascending=[True, True, True])
for i in df1['Регион']:
    if i not in li:
        ranks.append(1)
        li.append(i)
...
df1['Число больничных органзаций'] = df1['Число больничных органзаций'].fillna(0).astype('int')
df1['Год'] = df1['Год'].fillna(0).astype('int')
plt.style.use('dark_background')
fig, ax = plt.subplots(figsize=(15,8))
df1.to_csv(r'C:\Users\USER\Desktop\Life_Project\Hosp_Amount\data\Hosp_anim.csv') 