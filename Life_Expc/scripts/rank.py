...
file2 = r'C:\Users\USER\Desktop\Life_Project\Life_Expc\data\life_X_fin.csv'
...
def ml_new_regions(region_to_test, region_predict):
    ...
    values = df_vko['Ожидаемый возраст населения при рождении'].values.tolist()
    k = 0
    c = 0
    ...
    list_res = []
    list_yrs = []
    filtered_df = df.loc[(df['Регион'] == region_predict) & (df['Год'] == 2021), 'Ожидаемый возраст населения при рождении']
    ...
    kval = 0
    for i in list_yrs:
        df['Ожидаемый возраст населения при рождении'] = np.where((df['Регион'] == region_predict) & (df['Год'] == i), list_res[kval], df['Ожидаемый возраст населения при рождении'])
        kval = kval +1
...
li = []
ranks = []
df1 = df1.sort_values(by=['Год', 'Регион', 'Ожидаемый возраст населения при рождении'], ascending=[True, True, True])
for i in df1['Регион']:
    if i not in li:
        ranks.append(1)
        li.append(i)
...
df1['Ожидаемый возраст населения при рождении'] = df1['Ожидаемый возраст населения при рождении'].fillna(0).astype('float')
df1['Год'] = df1['Год'].fillna(0).astype('int')
plt.style.use('dark_background')
fig, ax = plt.subplots(figsize=(15,8))
df1.to_csv(r'C:\Users\USER\Desktop\Life_Project\Life_Expc\data\anim.csv') 